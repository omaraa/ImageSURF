...
object Training {
    private fun <T> indexOf(haystack: Array<T>, needle: T): Int {
    ...
    fun getSelectedFeaturesReader(optimalFeatures: Array<FeatureCalculator>, allFeatures: Array<FeatureCalculator>,
        ...
        val optimisedTrainingExamples: Array<Any?> = arrayOfNulls<Any>(numFeatures + 1)
        ...
        return when (pixelType) {
            PixelType.GRAY_8_BIT -> ImageFeatures.ByteReader(optimisedTrainingExamples as Array<ByteArray>, numFeatures)
            PixelType.GRAY_16_BIT -> ImageFeatures.ShortReader(optimisedTrainingExamples as Array<ShortArray>, numFeatures)
        }
    }

    fun getTrainingExamples(labelFiles: Array<File>, unlabelledFiles: Array<File>, rawImageFiles: Array<File>,
                            featureFiles: Array<File>, imageSurfDataPath: File, random: Random,
                            progressListener: ProgressListener?, examplePortion: Int,
                            saveCalculatedFeatures: Boolean, pixelType: PixelType,
                            selectedFeatures: Array<FeatureCalculator>): Array<Any> {
        ...
        val examples: List<List<FeatureImage<Any>>> = rawImageFiles.indices
                .map { imageIndex ->
                    ...
                    if (calculatedFeatures && saveCalculatedFeatures && !savedFeatures.containsAll(imageFeatures.easilyComputedFeatures)) {
                        writeFeatures(imageSurfDataPath, progressListener, imageIndex, numImages, featureFiles, imageFeatures)
                    }
                    ...
                    (selectedFeatures.mapIndexed { featureIndex, featureCalculator ->
    ...
    private fun writeFeatures(imageSurfDataPath: File, progressListener: ProgressListener, imageIndex: Int, numImages: Int, featureFiles: Array<File>, imageFeatures: ImageFeatures) {
    ...
    private fun getImageFeatures(featureFiles: Array<File>, imageIndex: Int, progressListener: ProgressListener, numImages: Int, rawImage: ImagePlus, pixelType: PixelType): Pair<ImageFeatures, Collection<FeatureCalculator>> {
        ...
        if (!featureFiles[imageIndex].exists()) {
        ...
        if (imageFeatures.pixelType != pixelType)
            throw RuntimeException("Training images must all be either 8 or 16 bit greyscale format. " +
                    "${featureFiles[imageIndex].name} is ${imageFeatures.pixelType}, expected $pixelType")
        return Pair(imageFeatures, savedFeatures)
    }

    private fun getRawTrainingImage(imageFile: File, numChannels: Int): ImagePlus {
        val rawImage = getImagePlus(imageFile)

        if (rawImage.nFrames * rawImage.nSlices > 1)
            throw RuntimeException("Training image ${rawImage.title} not valid. Images must be single plane.")

        if (rawImage.nChannels != numChannels)
            throw RuntimeException("Training image ${rawImage.title} not valid. Image has ${rawImage.nChannels}" +
    ...
    private fun selectExamplePixelIndices(labelledPixels: List<IntArray>, random: Random, examplePortion: Int): List<IntArray> {
        ...
        val totalLabelledPixels = labelledPixels.map { it.fold(0) { acc, i -> acc + i } }.fold(0) { acc, i -> acc + i }
